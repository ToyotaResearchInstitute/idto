 ...
 private:
  void CreatePlantModel(MultibodyPlant<double>* plant) const final {
    ...
    std::string robot_file = idto::FindIDTOResourceOrThrow(
        "traj_opt/examples/models/j2s7s300_arm_sphere_collision_v2.sdf");
    ...
    std::string manipuland_file =
        idto::FindIDTOResourceOrThrow("traj_opt/examples/models/box_15cm.sdf");
  ...
  void CreatePlantModelForSimulation(
    ...
    std::string robot_file = idto::FindIDTOResourceOrThrow(
        "traj_opt/examples/models/j2s7s300_arm_hydro_collision.sdf");
    ...
    std::string manipuland_file = idto::FindIDTOResourceOrThrow(
        "traj_opt/examples/models/box_15cm_hydro.sdf");
    Parser(plant).AddAllModelsFromFile(manipuland_file);
...
int main() {
  ...
  example.RunExample("traj_opt/examples/dual_jaco.yaml");
  return 0;
}
