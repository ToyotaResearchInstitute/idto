  ...
  void CreatePlantModel(MultibodyPlant<double>* plant) const {
    ...
    if (FLAGS_geometry == "rectangle") {
      urdf_file = idto::FindIDTOResourceOrThrow(
          "traj_opt/examples/models/spinner_rectangle.urdf");
    } else if (FLAGS_geometry == "capsule") {
      urdf_file = idto::FindIDTOResourceOrThrow(
          "traj_opt/examples/models/spinner_capsule.urdf");
    } else if (FLAGS_geometry == "square") {
      urdf_file = idto::FindIDTOResourceOrThrow(
          "traj_opt/examples/models/spinner_square.urdf");
...
int main(int argc, char* argv[]) {
  ...
  idto::traj_opt::examples::frictionless_spinner::FrictionlessSpinnerExample
      example;
  example.RunExample("traj_opt/examples/frictionless_spinner.yaml");
  return 0;
}
